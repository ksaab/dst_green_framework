...
local ShopScreen = Class(Widget, function(self, owner)
    ...
    self.closeButton = self.root:AddChild(TEMPLATES.StandardButton(function() self:Close() end, STRINGS.GF.HUD.JOURNAL.BUTTONS.CLOSE, {180, 60}))
    self.closeButton:SetPosition(0, -250)

    self:Hide()

    --owner:ListenForEvent("gfShopOpen", function(inst, list) self:Open(list) end)
    --owner:ListenForEvent("gfShopClose", function(inst) self:Close() end)
end)

function ShopScreen:Open(list)
    local function RowConstructor(context, index)
        local widget = ShopItem(self.owner)
        return widget
    end
