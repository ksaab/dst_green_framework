        ...
        if act.doer:HasTag("player") then
            ...
            if item ~= nil then
                ...
                if item.components.gfspellitem then
                    local itemSpell = item.components.gfspellitem:GetItemSpell()
                    return itemSpell 
                        and act.doer.components.gfspellcaster:CanCastSpell(spell)
                        and item.components.gfspellitem:CanCastSpell(spell)
...
AddAction("GFCHANGEITEMSPELL", "Change Spell", function(act)
    ...
    then
        if act.invobject.components.gfspellitem:ChangeSpell() then
            print(("GFCHANGEITEMSPELL: Change spell for %s to %s"):format(tostring(act.invobject), act.invobject.components.gfspellitem:GetItemSpell()))
...
AddComponentAction("POINT", "gfspellitem", function(inst, doer, pos, actions, right)
    ...
    if right then
        if doer.replica.gfspellcaster then
            local itemSpell = inst.replica.gfspellitem:GetItemSpell()
