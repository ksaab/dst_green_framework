...
function GFPointer:Create(pointer)
...
function GFPointer:RecalculateTarget()
    ...
    if TheInput:ControllerAttached() then
        --controller
        local playerRot = self.inst.Transform:GetRotation() * DEGREES
        ...
        if self.prefersTarget then
            ...
            for i, ent in pairs(ents) do
                if ent ~= self.inst and ent.entity:IsVisible() and CanEntitySeeTarget(self.inst, ent) then
        ...
        if self.prefersTarget then
            cursorEntity = TheInput:GetWorldEntityUnderMouse()
            if cursorEntity ~= nil 
                and cursorEntity:IsValid() 
                and not cursorEntity:HasTag("FX")
                and not cursorEntity:HasTag("NOCLICK")
            then
                cursorPos = Vector3(cursorEntity.Transform:GetWorldPosition())
            end
        end
